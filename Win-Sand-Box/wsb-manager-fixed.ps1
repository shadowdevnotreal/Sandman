<#
Windows Sandbox .wsb Manager (v2)
- Create / List / Edit / Validate / Modify / Launch
- Defensive, minimal dependencies, single file
#>
...
function Action-List {
  Clear-Host; Write-Host "Files in $Workspace" -ForegroundColor Cyan
  $list = List-WsbFiles
  if (-not $list) { Write-Host "None."; return }
  $i=1; foreach ($f in $list) { Write-Host ("[{0}] {1}  ({2})" -f $i,$f.Name,$f.LastWriteTime); $i++ }
}

function Action-Edit {
  Clear-Host; $list = List-WsbFiles
  if (-not $list) { Write-Host "No files to edit."; return }
  $files = $list.FullName
  for ($i=0; $i -lt $files.Length; $i++) { Write-Host ("[{0}] {1}" -f ($i+1),(Split-Path $files[$i] -Leaf)) }
  $sel = Read-Host "Choose #"; if ($sel -notmatch '^\d+$' -or $sel -lt 1 -or $sel -gt $files.Length) { return }
...
function Action-Validate {
  ...
  if ($errs.Count -eq 0) {
    ...
    # show summary
    foreach ($k in "Networking","VGpu","MemoryInMB","AudioInput","VideoInput","PrinterRedirection","ClipboardRedirection","ProtectedClient") {
      $v = $res.Doc.Configuration.$k.'#text'; if ($v) { Write-Host ("  {0} = {1}" -f $k,$v) }
    }
    if ($res.Doc.Configuration.MappedFolders) {
      foreach ($mf in $res.Doc.Configuration.MappedFolders.MappedFolder) {
        Write-Host ("  Map: {0} (RO={1})" -f $mf.HostFolder.'#text',$mf.ReadOnly.'#text')
      }
    }
  } else {
    Write-Host "INVALID ❌" -ForegroundColor Red
    $errs | ForEach-Object { Write-Host "  - $_" -ForegroundColor Red }
    # quick remediation: create missing folders?
    $missing = $errs | Where-Object { $_ -like "HostFolder does not exist:*" } |
               ForEach-Object { ($_ -split ":",2)[1].Trim() } | Select-Object -Unique
...
function Action-Modify {
  ...
  switch ($choice) {
    "Add mapped folder" {
      $hf = Read-Host "HostFolder path (e.g., C:\myshare)"; if (-not $hf) { break }
    ...
    "Remove mapped folder(s)" {
      if ($null -eq $doc.Configuration.MappedFolders) { Write-Host "None."; break }
      ...
      if ($x -eq 'all') { $null = $doc.Configuration.RemoveChild($doc.Configuration.MappedFolders) }
    ...
    "Set Memory" {
      ...
      if (-not ($m -as [int])) { Write-Host "Invalid"; break }
      $m = [int]$m; if ($m -lt $MinMemoryMB -or $m -gt $MaxMemoryMB) { Write-Host "Out of range"; break }
    ...
    "Set Networking" {
      $v = Read-Host "Networking (Default/Disable)"; if ($Allowed.Networking -notcontains $v) { Write-Host "Invalid"; break }
    ...
    "Set vGPU" {
      $v = Read-Host "VGpu (Default/Disable)"; if ($Allowed.VGpu -notcontains $v) { Write-Host "Invalid"; break }
      if ($null -eq $doc.Configuration.VGpu) { $e=$doc.CreateElement("VGpu"); $null=$doc.Configuration.AppendChild($e) }
      $doc.Configuration.VGpu = $v
    }
    "Set AudioInput" {
      $v = Read-Host "AudioInput (Default/Enable/Disable)"; if ($Allowed.AudioInput -notcontains $v) { Write-Host "Invalid"; break }
    ...
    "Set VideoInput" {
      $v = Read-Host "VideoInput (Default/Enable/Disable)"; if ($Allowed.VideoInput -notcontains $v) { Write-Host "Invalid"; break }
    ...
    "Toggle PrinterRedirection" {
      ...
      $doc.Configuration.PrinterRedirection = $new; Write-Host "PrinterRedirection=$new"
    }
    "Toggle ClipboardRedirection" {
      ...
      $doc.Configuration.ClipboardRedirection = $new; Write-Host "ClipboardRedirection=$new"
    }
    "Toggle ProtectedClient" {
      ...
      $doc.Configuration.ProtectedClient = $new; Write-Host "ProtectedClient=$new"
    }
    default { return }
  }
...
function Main-Loop {
  while ($true) {
    Write-Host ""
    Write-Host "=== Windows Sandbox .wsb Manager (v2) ===" -ForegroundColor Yellow
    ...
    Write-Host "[5] Modify (toggle options / folders)"
